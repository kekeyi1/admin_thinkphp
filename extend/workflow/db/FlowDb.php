...
class FlowDb
{
     ...
     *@param $run_flow_process 工作流ID
     **/
    public static function end_this_process($run_process,$check_con,$run_id)
    {
        return Db::name('run_process')->where('run_id',$run_id)->where('run_flow_process',$run_process)->update(['status'=>2,'remark'=>$check_con,'bl_time'=>time()]);
    }
