	...
	public static function addWorkflowProcess($wf_id,$wf_process,$run_id,$uid,$todo = '')
	{
		...
		if($todo == ''){
			...
			if($wf_process['auto_person']==6){ //事务接收者 2020年1月17日15:28:37
                ...
                $user_info = UserDb::GetUserData($user_id);
                $sponsor_text = $user_info['true_name'];
                $sponsor_ids = $user_info['uid'];
			}
		}else{
			$todo = explode("*%*",$todo);
			$sponsor_text = $todo[1];
...
//		$result = Db::name($wf_type)->where('id','eq',$wf_fid)->update(['status'=>$status,'uptime'=>time()]);
		$result = Db::name($wf_type)->where('id','eq',$wf_fid)->update(['flow_state'=>$status]);
		 if(!$result){
            return  false;
        }
        return $result;
     ...
     * @param $wf_type 业务表名
     */
    public static function wfInfo($wf_fid, $wf_type, $userinfo)
    {
        $workflow = [];
        require(BEASE_URL . '/config/config.php');
        //根据表信息，判断当前流程是否还在运行
        ...
        if ($count > 0) {
            //获取当前运行的信息
            $result = Db::name('run')->where('from_id', 'eq', $wf_fid)->where('from_table', 'eq', $wf_type)->where('is_del', 'eq', 0)->find();
            $info_list = Db::name('run_process')
                ->where('run_id', 'eq', $result['id'])
                ->where('run_flow', 'eq', $result['flow_id'])
                ->where('run_flow_process', 'in', $result['run_flow_process'])
            ...
            if (count($info_list) == 0) {
                $info_list[0] = Db::name('run_process')->where('run_id', 'eq', $result['id'])->where('run_flow', 'eq', $result['flow_id'])->where('run_flow_process', 'eq', $result['run_flow_process'])->find();
            }
            ...
            if ($result) {
                ...
                $workflow ['flow_name'] = FlowDb::GetFlowInfo($result['flow_id']);
                $workflow ['process'] = ProcessDb::GetProcessInfo($info['run_flow_process'], $result['id']);
                $workflow ['nexprocess'] = ProcessDb::GetNexProcessInfo($wf_type, $wf_fid, $info['run_flow_process'], $result['id']);
