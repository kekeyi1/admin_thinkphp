...
class BackFlow{
	...
	public function doTask($config,$uid) {
		...
		if($back){//第一步
            ...
            //又是一个坑，退回和发送都走了事物处理机制
            if ($config['wf_type'] == 'subject_matter_performance') {
                $res = Db::name('subject_matter_performance')->where('id', $config['wf_fid'])->update(['state' => 1]);
                Db::name('subject_matter_performance_distribution')->where('performance_id', $config['wf_fid'])->update(['is_sure' => 0, 'weight' => 0, 'amount' => 0,'state'=>0]);
            }
			if(!$run_log){
